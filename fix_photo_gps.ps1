# Script to diagnose and fix issues with photo GPS data
Write-Host "Diagnosing Photo GPS Data Issues" -ForegroundColor Green

# Execute Python script to check photo details
$pythonScript = @"
import sqlite3
import json
import os
...
# Save Python script
$pythonScriptPath = Join-Path (Get-Location) "check_photos.py"
$pythonScript | Out-File -Encoding utf8 $pythonScriptPath

Write-Host "Created check_photos.py script to analyze photo data"
...
Write-Host "1. If photos exist but have no GPS data:"
Write-Host "   - Check if your photos actually contain GPS coordinates in their EXIF data"
Write-Host "   - Make sure process_photos.py correctly extracts GPS data"
Write-Host "`n2. If no photos are found:"
Write-Host "   - Check the source directories in your libraries"
Write-Host "   - Run process_photos.py to scan for photos"
Write-Host "`n3. To rebuild the photo database:"
Write-Host "   - Run: python process_photos.py --rebuild" -ForegroundColor Cyan
Write-Host "   - Or use the rebuild_library.ps1 script if available"
